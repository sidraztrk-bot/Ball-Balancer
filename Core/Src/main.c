...
/* Private define ------------------------------------------------------------*/
/* USER CODE BEGIN PD */
#define CONTROL_LOOP_MS 20       // فترة حلقة التحكم بالملّي ثانية (50Hz)
...
/* USER CODE BEGIN PV */
uint8_t rx_byte;      // بايت واحد نستقبله كل مرة
char rx_buffer[16];         // بافر لتجميع الرسالة
int rx_index = 0;           // مؤشر الكتابة
uint8_t rx_data[20];
//uint8_t temp[2];
float ballX_cm = 0.0f, ballY_cm = 0.0f;
float angleX, angleY ;

float desiredPositionX = 5.0f;  // X-axis reference signal
float desiredPositionY = 3.0f;  // Y-axis reference signal
...
{
	if (huart->Instance == USART2) { // تأكدي انه USART2
	        char c = (char)rx_byte;
	        if (c == '\n') { // نهاية الرسالة
	            rx_buffer[rx_index] = '\0';  // انهي النص
	            rx_index = 0;                // حضري للبداية الجديدة
	            char *token = strtok(rx_buffer, ",");
				if (token !=NULL){
					ballX_cm = atoi(token)/100;
					token = strtok(NULL, ",");
					if(token!=NULL){
						ballY_cm = atoi(token)/100;
						ballDetected = 1; // ✅ صار في كرة
				...
				memset(rx_buffer, 0, sizeof(rx_buffer));
	                    ...
	                    if (rx_index < sizeof(rx_buffer) - 1) {
	                        ...
	                        // overflow → صفري المؤشر
	                        rx_index = 0;
	                        memset(rx_buffer, 0, sizeof(rx_buffer));
	                    }
	                }

	                // جددي الاستقبال للبايت القادم
...
positionControllerX.Kp = 5.0;
positionControllerX.Ki = 0.1;
positionControllerX.Kd = 1.0;

positionControllerY.Kp = 5.0;
positionControllerY.Ki = 0.1;
positionControllerY.Kd = 1.0;

  PID_Init(&positionControllerX, 0.02f); // 20ms
  PID_Init(&positionControllerY, 0.02f); // 20ms
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	  ...
	  if ((HAL_GetTick() - lastBallUpdate) > 20) {
	      ballDetected = 0; // ❌ ما وصل شي من الكاميرا
	  }

	  if (ballDetected) {
		  // TARGET POSITIONS - Change these to your desired setpoints
		  errorPositionX = desiredPositionX - ballX_cm;
		  errorPositionY = desiredPositionY - ballY_cm;

		  if ((HAL_GetTick() - loopTimer) >= 20) {
			  ...
			  pulsex = mapping(angleX, -20, 20, 1100, 1700);
			  pulsey = mapping(angleY, -20, 20, 1200, 1900);
